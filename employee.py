...
class employeeClass:
    def __init__(self, root):
        ...
        lbl_naem = Label(self.root,text = "Name",font = ("goudy old style",15),bg = "white").place(x=50,y=190)
        ...
        lbl_adress = Label(self.root,text = "Address",font = ("goudy old style",15),bg = "white").place(x=50,y=270)
    ...
    def add(self):
        ...
        try:
            if self.var_emp_id.get()=="":
                ...
                row = cur.fetchone()
                if row!=None:
                    messagebox.showerror("Error","This EmployeeID already assigned, try differen",parent = self.root)
                else:
        ...
        try:
            if self.var_emp_id.get() == "":
                ...
                if row is None:
                    ...
                    cur.execute("""
                        UPDATE employee 
                        SET name=?, email=?, gender=?, contact=?, dob=?, doj=?, pass=?, utype=?, address=?, salary=? 
                        WHERE eid = ?
                    """, (
                        self.var_name.get(),
                        self.var_email.get(),
